*NDC data - GET STATUTIL DATA READY
*link to data
...
log using Stateutil_`today'.log, replace
...
gen GENERICANDA = 0
replace GENERICANDA = 1 if strpos(marketingcategoryname,"ANDA")>0
gen cat = proprietaryname+labelername

save "`a'/cat_product.dta",replace
clear

use "`a'/GENERIC_NDA_LISTING_raw.dta"
drop if strmatch(ProprietaryName,"") | strmatch(ProprietaryName,"Proprietary Name")
gen cat = ProprietaryName + NDAApplicantName
*The file that has the GENERIC NDA Listing

duplicates drop cat, force
save "`a'/GENERIC_NDA_LISTING_cat.dta",replace
clear

use "`a'/cat_product.dta"
merge m:1 cat using `a'/GENERIC_NDA_LISTING_cat.dta, keepusing(cat)

gen GENERICNDA = 0
replace GENERICNDA = 1 if _merge==3

gen GENERIC=0
replace GENERIC = 1 if(GENERICNDA == 1 | GENERICANDA == 1)
...
save "new_ndc_package.dta",replace
...
gen ANDA = 0
replace ANDA = 1 if strpos(marketingcategory,"ANDA")>0

gen cat = proprietaryname+firm_name
merge m:1 cat using `a'/GENERIC_NDA_LISTING_cat.dta, keepusing(cat)
...
gen GENERIC = 0
replace GENERIC = 1 if(ANDA==1 | GENERIC==1)
